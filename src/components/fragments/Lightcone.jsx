...
const lcToId = {
  ...
  "Make Farewells More Beautiful": 23040,
  ...
  "Destiny's Threads Forewoven": 21039,
  "Flames Afar": 21038,
  "Final Victor": 21037,
...
const idToLc = Object.fromEntries(
  Object.entries(lcToId).map(([name, id]) => [id, name]),
);
...
export default function Lightcone() {
  ...
  const [id, setId, level, setLevel, rank, setRank, promotion, setPromotion] =
    useLightconeStore(
      useShallow((state) => [
        state.id,
        state.setId,
  ...
  const filteredId = listId.filter((lcId) => {
    ...
    const translatedFilters = filterBaseType.map(
      (type) => pathMap[type] || type,
    );
    const matchesBaseType =
      filterBaseType.length > 0 ? translatedFilters.includes(lc?.path) : true;
  ...
  return (
    <div className="flex flex-col px-3">
      ...
      <Dialog>
        <DialogTrigger className="bg-black text-white dark:bg-white dark:text-black rounded-md px-5 py-2 font-semibold">
          Select lightcone
        </DialogTrigger>
        <DialogContent
              ...
              <div className="flex gap-1 mt-2">
                ...
                <div
                  className={`filter text-white text-2xl font-semibold flex items-center justify-center cursor-pointer ${
                    filterRankType.includes("CombatPowerLightconeRarity5")
                      ? "bg-black/75 dark:bg-white/15 border border-black dark:border-white"
                      : "border"
                  }`}
                  onClick={() =>
                    handleRankChange("CombatPowerLightconeRarity5")
                  }
                >
                  5*
                </div>
                <div
                  className={`filter text-white text-2xl font-semibold flex items-center justify-center cursor-pointer ${
                    filterRankType.includes("CombatPowerLightconeRarity4")
                      ? "bg-black/75 dark:bg-white/15 border border-black dark:border-white"
                      : "border"
                  }`}
                  onClick={() =>
                    handleRankChange("CombatPowerLightconeRarity4")
                  }
                >
                  4*
                </div>
                <div
                  className={`filter text-white text-2xl font-semibold flex items-center justify-center cursor-pointer ${
                    filterRankType.includes("CombatPowerLightconeRarity3")
                      ? "bg-black/75 dark:bg-white/15 border border-black dark:border-white"
                      : "border"
                  }`}
                  onClick={() =>
                    handleRankChange("CombatPowerLightconeRarity3")
                  }
                >
                  3*
                </div>
            ...
            {filteredId.reverse().map((lcId) => {
              ...
              return (
                <DialogClose asChild key={lcId}>
                  ...
                  >
                    <img
                      src={`https://static.nanoka.cc/assets/hsr/lightconemediumicon/${lcId}.webp`}
                      alt={displayName}
                    />
      ...
      <div className="grid grid-cols-2">
        <div>
          <img
            className="w-[300px]"
            src={`https://static.nanoka.cc/assets/hsr/lightconemediumicon/${id}.webp`}
            alt={id}
          />
...
function Path({ path, filter, base }) {
  return (
    <div
      className={`filter cursor-pointer ${base.includes(path) ? "bg-black/75 dark:bg-white/15 border border-black dark:border-white" : "border"}`}
      onClick={() => filter(path)}
    >
      <img
        src={`https://cdn.neonteam.dev/neonteam/assets/spriteoutput/avatarprofessiontattoo/profession/BgPaths${path}.webp`}
        alt={path}
      />
    </div>
  );
