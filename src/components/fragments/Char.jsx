...
export default function Char({ isEdit = false, isTingyun = false }) {
  ...
  const handleBaseChange = (baseType) => {
    setFilterBaseType((prevFilters) =>
      prevFilters.includes(baseType)
        ? prevFilters.filter((type) => type !== baseType)
        : [...prevFilters, baseType],
    );
  };

  const handleDamageChange = (damageType) => {
    setFilterDamageType((prevFilters) =>
      prevFilters.includes(damageType)
        ? prevFilters.filter((type) => type !== damageType)
        : [...prevFilters, damageType],
    );
  };

  const handleRankChange = (rankType) => {
    setFilterRankType((prevFilters) =>
      prevFilters.includes(rankType)
        ? prevFilters.filter((type) => type !== rankType)
        : [...prevFilters, rankType],
    );
  };
  ...
  const filteredId = listId.filter((id) => {
    ...
    const translatedPathFilters = filterBaseType.map(
      (type) => pathMap[type] || type,
    );
    const matchesBaseType =
      filterBaseType.length > 0
        ? translatedPathFilters.includes(character?.path)
        : true;

    const matchesDamageType =
      filterDamageType.length > 0
        ? filterDamageType.includes(character?.element)
        : true;

    const matchesRankType =
      filterRankType.length > 0
        ? filterRankType.some((r) => r.includes(character?.rarity?.toString()))
        : true;

    return (
      matchesSearch && matchesBaseType && matchesDamageType && matchesRankType
    );
  });
  ...
  return (
    <div className="flex flex-col px-3">
      ...
      <Dialog>
        <DialogTrigger
          disabled={isEdit}
          className="bg-black text-white dark:bg-white dark:text-black rounded-md px-5 py-2 font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Select character
        </DialogTrigger>
        <DialogContent
              ...
              <div className="flex gap-1 mt-2">
                ...
                {element.map((ele) => (
                  <Element
                    key={ele}
                    ele={ele}
                    filter={handleDamageChange}
                    damage={filterDamageType}
                  />
                ))}
                <div
                  className={`filter text-white text-2xl font-semibold flex items-center justify-center cursor-pointer ${
                    filterRankType.includes("CombatPowerAvatarRarityType5")
                      ? "bg-black/75 dark:bg-white/15 border border-black dark:border-white"
                      : "border"
                  }`}
                  onClick={() =>
                    handleRankChange("CombatPowerAvatarRarityType5")
                  }
                >
                  5*
                </div>
                <div
                  className={`filter text-white text-2xl font-semibold flex items-center justify-center cursor-pointer ${
                    filterRankType.includes("CombatPowerAvatarRarityType4")
                      ? "bg-black/75 dark:bg-white/15 border border-black dark:border-white"
                      : "border"
                  }`}
                  onClick={() =>
                    handleRankChange("CombatPowerAvatarRarityType4")
                  }
                >
                  4*
                </div>
            ...
            {filteredId.reverse().map((charId) => {
              ...
              return (
                <DialogClose asChild key={charId}>
                  ...
                  >
                    <img
                      src={`https://cdn.neonteam.dev/neonteam/assets/spriteoutput/avatarshopicon/avatar/${charId}.webp`}
                      alt={displayName}
                    />
      ...
      <div className="grid grid-cols-2">
        <img
          className="w-[300px]"
          src={`https://cdn.neonteam.dev/neonteam/assets/spriteoutput/avatarshopicon/avatar/${id}.webp`}
          alt={id}
        />
        {id && (
          <div className="flex flex-col gap-5">
            <div>
              ...
              <Slider
                ...
                value={[level]}
                onValueChange={([val]) =>
                  setLevel(val < 10 ? 1 : Math.floor(val / 10) * 10)
                }
                disabled
              />
            </div>
            ...
            <div>
              ...
              <Slider
                ...
                step={1}
                value={[promotion]}
                onValueChange={([val]) => setPromotion(val)}
                disabled
              />
            </div>
            <div>
              <span>Energy: {isTingyun ? 90 : Math.floor(energy / 100)}%</span>
              <Slider
                className={`mt-1 ${isTingyun ? "cursor-not-allowed" : "cursor-pointer"}`}
                defaultValue={[5000]}
                max={10000}
                min={0}
                step={100}
                value={[isTingyun ? 9000 : energy]}
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                id="trace"
                checked={trace}
                onCheckedChange={(val) => setTrace(val)}
              />
              <label
                htmlFor="trace"
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                id="technique"
                checked={isTingyun ? false : technique}
                onCheckedChange={(val) => setTechnique(val)}
                disabled = {isTingyun}
              />
              <label
                htmlFor="technique"
...
function Path({ path: p, filter, base }) {
  return (
    <div
      className={`filter cursor-pointer ${base.includes(p) ? "bg-black/75 dark:bg-white/15 border border-black dark:border-white" : "border"}`}
      onClick={() => filter(p)}
    >
      <img
        src={`https://cdn.neonteam.dev/neonteam/assets/spriteoutput/avatarprofessiontattoo/profession/BgPaths${p}.webp`}
        alt={p}
      />
    </div>
  );
...
function Element({ ele, filter, damage }) {
  return (
    <div
      className={`filter cursor-pointer ${damage.includes(ele) ? "bg-black/75 dark:bg-white/15 border border-black dark:border-white" : "border"}`}
      onClick={() => filter(ele)}
    >
      <img
        className="w-full"
        src={`https://cdn.neonteam.dev/neonteam/assets/spriteoutput/ui/nature/iconattributemiddle/IconAttribute${ele}.webp`}
        alt={ele}
      />
    </div>
  );
